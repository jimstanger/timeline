...
class Modern_Timeline {
    ...
    public function enqueue_styles() {
        wp_enqueue_style( 'modern-timeline-css', plugin_dir_url( __FILE__ ) . 'timeline.css', array(), '2.4.0' );
    }
    ...
    public function render_timeline_shortcode( $atts ) {
        ...
        $default_design = get_option( 'modern_timeline_design', 'alternating-vertical' );
        $allowed_designs = array( 'alternating-vertical', 'horizontal-flag-cards' );
        
        if ( ! empty( $atts['design'] ) && in_array( $atts['design'], $allowed_designs ) ) {
            $design = $atts['design'];
        } else {
        ...
        // ---------------------------------------------------------
        if ( $design === 'horizontal-flag-cards' ) {
            echo '<div class="mt-timeline-horizontal-container">';
            echo '<div class="mt-h-items-wrapper">';
            ...
            while ( $query->have_posts() ) {
                ...
                $bg_color = get_post_meta( get_the_ID(), '_timeline_color', true );
                if ( ! $bg_color ) $bg_color = '#00B8D4'; 
                ?>
                    ...
                    <div class="mt-h-card">
                        <div class="mt-h-icon">
                            <?php if ( has_post_thumbnail() ) {
                                ...
                                echo '<span style="color:white; font-size: 24px;">★</span>';
                            } ?>
                        </div>
        ...
        // ---------------------------------------------------------
        // DESIGN 1: ALTERNATING VERTICAL (Original)
        // ---------------------------------------------------------
        } else {
            ...
            while ( $query->have_posts() ) {
                ...
                <div class="mt-timeline-item <?php echo $side_class; ?>" style="--mt-item-color: <?php echo esc_attr($bg_color); ?>;">
                    <div class="mt-content">
                        ...
                        <?php if ( has_post_thumbnail() ) : ?>
                            <div class="mt-image"><?php the_post_thumbnail('medium'); ?></div>
                        <?php endif; ?>
                    </div>
                </div>
                <?php
